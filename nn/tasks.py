...
from fontTools.feaLib.ast import ElidedFallbackNameID
...
from .modules.conv import EMAConv
from .Extramodule.head import *
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        elif m in ((WorldDetect,
                    ImagePoolingAttn) + DETECT_CLASS + V10_DETECT_CLASS):
            args.append([ch[x] for x in f])

        elif m is SFHC:
            dim = args[0]
            norm_layer_name = args[1] if len(args) > 1 else 'torch.nn.BatchNorm2d'
            if norm_layer_name == 'torch.nn.BatchNorm2d':
                norm_layer = nn.BatchNorm2d
            elif norm_layer_name == 'torch.nn.InstanceNorm2d':
                norm_layer = nn.InstanceNorm2d
            else:
                try:
                    norm_layer = eval(norm_layer_name)
                except NameError:
                    raise ValueError(f"Unknown norm_layer: {norm_layer_name}")
            args = [dim, norm_layer]
        else:
            c2 = ch[f]
