...
def pad_to_power_of_two(x, power=2):
    _, _, h, w = x.size()
    new_h = int(2 ** (h - 1).bit_length())
    new_w = int(2 ** (w - 1).bit_length())
...
class ConvolutionalGLU(nn.Module):
    def __init__(self, dim, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.) -> None:
        super().__init__()
        out_features = out_features or dim
        hidden_features = hidden_features or dim
        hidden_features = int(2 * hidden_features / 3)
        self.fc1 = nn.Conv2d(dim, hidden_features * 2, 1)
...
class SFHC(nn.Module):
    def __init__(
        ...
        self.cglu = ConvolutionalGLU(dim=self.dim)
