...
class DIM(nn.Module):
    def __init__(
        ...
        super(DIM, self).__init__()  # 调用 nn.Module 的构造函数
        self.dim = dim  # 特征维度
        self.head_num = head_num//4 # 注意力头数
        ...
        # 定义查询、键和值的卷积层
        self.q = nn.Conv2d(in_channels=dim, out_channels=group_chans, kernel_size=1, bias=qkv_bias, groups=group_chans)
        self.k = nn.Conv2d(in_channels=dim, out_channels=group_chans, kernel_size=1, bias=qkv_bias, groups=group_chans)
        self.v = nn.Conv2d(in_channels=dim, out_channels=group_chans, kernel_size=1, bias=qkv_bias, groups=group_chans)
        self.solve_x = nn.Conv2d(in_channels=dim, out_channels=dim//4, kernel_size=1)
