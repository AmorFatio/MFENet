...
def pytest_collection_modifyitems(config, items):
    """
    Modify the list of val items to exclude tests marked as slow if the --slow option is not specified.
    ...
    if not config.getoption("--slow"):
        # Remove the item entirely from the list of val items if it's marked as 'slow'
...
def pytest_sessionstart(session):
    ...
    val collection. It sets the initial seeds and prepares the temporary directory for the val session.
...
def pytest_terminal_summary(terminalreporter, exitstatus, config):
    ...
    This function is automatically called by pytest at the end of the entire val session. It removes certain files
    ...
    Args:
        ...
        exitstatus (int): The exit status of the val run.
