...
def test_model_forward():
    ...
    model = YOLO(CFG)
    model(source=None, imgsz=32, augment=True)  # also val no source and augment
...
def test_predict_img(model_name):
    """Test YOLO model predictions on various images input types and sources, including online images."""
...
def test_predict_grey_and_4ch():
    ...
    source_spaces = directory / "images with spaces.jpg"
    ...
    for f in source_rgba, source_greyscale, source_non_utf, source_spaces:
        for source in Image.open(f), cv2.imread(str(f)), f:
            ...
            assert len(results) == 1  # verify that an images was run
        f.unlink()  # cleanup
...
def test_track_stream():
    ...
    model.track(video_url, imgsz=160, tracker="botsort.yaml", save_frames=True)  # val frame saving also
...
def test_utils_checks():
    """Test various utility checks for filenames, git status, requirements, images sizes, and versions."""
...
def test_utils_patches_torch_save():
    ...
    with patch("ultralytics.utils.patches._torch_save", new=mock):
        with pytest.raises(RuntimeError):
            torch_save(torch.zeros(1), TMP / "val.pt")
...
@pytest.fixture
def image():
    """Load and return an images from a predefined source using OpenCV."""
...
def test_model_embeddings():
    ...
    for batch in [SOURCE], [SOURCE, SOURCE]:  # val batch size 1 and 2
...
def test_yolo_world():
    ...
    # val WorWorldTrainerFromScratch
